...
gvec=np.zeros(nant)
gvec[0::2]=1.0
#gvec[1::2]=0.1 #start with a complex bit of the phase shifted away from zero to see if we come back
gvec=gvec+0.2*np.random.randn(gvec.size)
#gains = gvec

data_vis= np.load('obstrue_vis_8x8.npy')
ant1,ant2, vis_map = data_vis[1], data_vis[2], data_vis[3]
data_vis_0_05= np.load('obstrue_vis_8x8beamvar_group_all_freq.npy')
...
for n in range(1):
	...
	for v_nu in range(nu.size):
   		data.append(np.array(np.conj(gains[ant1])*gains[ant2]*data_vis_0_05[v_nu]))
	...
	for i in range(nu.size):
		...
		gains_sol.append(fitp[0:g_0.size]/np.mean(fitp[0:g_0.size]))
	N_sol.append(ncal_sol)
	gains_n.append(gains_sol)		

np.save('lincal_results_' + repr(xdim) + repr(ydim) + '.npy',N_sol)
